...
am4core.ready(function() {
  ...
  title.text = "COVID-19 World Data";
  title.align = "left";
  ...
  function populateCountries(list) {
    var currentCountry;
    function setCurrentCountry(ee){
      currentCountry = ee;
      console.log("rang!!" + currentCountry)
    }
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      $("<td>").appendTo(tr).data("areaid", area.id).html(`<a href="./graphdisplay.html?ct=${area.name}" target="_blank" >${area.name}</a>`).on("click", setCurrentCountry(area.name));
